...
if (Result):
	RetOfStep = DL.SendCommand('Get DUKPT KSN (81-0B)_0208')
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("62 99 49 01 80 00 00 00")
...
# Set AMEX CT config
if (Result):
	RetOfStep = DL.SendCommand('Set AMEX CT config (60-03)')
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("00")
...
if (Result):
	...
	# Tag 5A
		Result = DL.Check_StringAB(mask5A, '5A A1 08 37 42 CC CC CC C1 00 9F')
		if Result == True and DL.Check_RXResponse("5A A1 08"):
			DL.SetWindowText("blue", "Tag 5A_Mask: PASS")
		else:
			DL.SetWindowText("red", "Tag 5A_Mask: FAIL")
			
		Result = DL.Check_StringAB(dec5A, '5A 08 37 42 45 00 17 21 00 9F')
	...
	# Tag 57
		...
		Result = DL.Check_StringAB(dec57, '57 13 37 42 45 00 17 21 00 9D 20 01 20 11 31 01 23 45 00 00 0F')
