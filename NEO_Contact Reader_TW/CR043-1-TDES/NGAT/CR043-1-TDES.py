	...
	if (RetOfStep):
		...
		if (Result):
			...
			if (Result):
				...
				DFEF4C = DL.GetTLV(alldata,"DFEF4C", 0)
				DFEF4D = DL.GetTLV(alldata,"DFEF4D", 0)
				decDFEF4D = DL.DecryptDLL(0,1, strKey, ksn, DFEF4D)	
			...
			# Tag 57
				...
				if Result == True and DL.Check_StringAB(DL.Get_RXResponse(2), '57 A1 11'):
					DL.SetWindowText("blue", "Tag 57_Mask: PASS")
				else:
					DL.SetWindowText("red", "Tag 57_Mask: FAIL")
			
				Result = DL.Check_StringAB(dec57, '57 11 47 61 73 90 01 01 00 10 D2 01 22 01 01 23 45 67 89')
				if Result == True and DL.Check_StringAB(DL.Get_RXResponse(2), '57 C1 18'):
			...
			# Tag 5A
				...
				if Result == True and DL.Check_StringAB(DL.Get_RXResponse(2), '5A A1 08'):
					DL.SetWindowText("blue", "Tag 5A_Mask: PASS")
				else:
					DL.SetWindowText("red", "Tag 5A_Mask: FAIL")
			
				Result = DL.Check_StringAB(dec5A, '5A 08 47 61 73 90 01 01 00 10')
				if Result == True and DL.Check_StringAB(DL.Get_RXResponse(2), '5A C1 10'):
					DL.SetWindowText("blue", "Tag 5A_Enc: PASS")
				else:
					DL.SetWindowText("red", "Tag 5A_Enc: FAIL")
					
			# Tag DFEF4C/ DFEF4D
				Result = DL.Check_StringAB(DFEF4C, '00 24 00 00 00 00')
				if Result == True:
