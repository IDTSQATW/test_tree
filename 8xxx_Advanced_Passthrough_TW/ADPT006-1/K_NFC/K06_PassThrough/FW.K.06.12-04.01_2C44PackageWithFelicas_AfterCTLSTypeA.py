...
T0240 = []
T0240.append(["ACT-Only CTLS","02 40 1E 9F 02 06 00 00 00 00 02 00 9C 01 00 DF EF 37 01 02",["02"],True])
T0240.append(["ACT-CTLS+MSR","02 40 1E 9F 02 06 00 00 00 00 02 00 9C 01 00 DF EF 37 01 03",["02"],False])
T0240.append(["ACT-CTLS+CT","02 40 1E 9F 02 06 00 00 00 00 02 00 9C 01 00 DF EF 37 01 06",["02 63 00 00","02"],True])
T0240.append(["ACT-All interface","02 40 1E 9F 02 06 00 00 00 00 02 00 9C 01 00 DF EF 37 01 07",["02 63 00 00","02"],False])
...
for Case in T0240:
	if (DeviceType == 14 or DeviceType == 15):
		#Type A
		if (Case[3]):
			if(Robot):DL.Robot_CTLSCard(7)
			else:DL.ShowMessageBox("Message","Please tap any CTLS typeA card",0)
			NUM = len(Case[2])
			if(False==Pro_SendCmd().RunCK_1( Pro_SendCmd().Run_1(NUM,Case[0],Case[1],50000,False,DL),Case[2],DL)):DL.fails=DL.fails+1
