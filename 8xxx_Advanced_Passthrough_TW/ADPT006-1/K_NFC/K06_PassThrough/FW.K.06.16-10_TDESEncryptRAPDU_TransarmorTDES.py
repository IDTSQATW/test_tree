...
C2C03 = []
C2C03.append(["PPSE ","2C 03 00 A4 04 00 0E 32 50 41 59 2E 53 59 53 2E 44 44 46 30 31 00  ","56 69 56 4F 74 65 63 68 32 00 2C00 003A 0028** 9000 000A** 0000","00 25 6F 23 84 0E 32 50 41 59 2E 53 59 53 2E 44 44 46 30 31 A5 11 BF 0C 0E 61 0C 4F 07 A0 00 00 00 04 10 10 87 01 01"])
C2C03.append(["Select ","2C 03 00 A4 04 00 07 A0 00 00 00 04 10 10 00 ","56 69 56 4F 74 65 63 68 32 00 2C00 0032 0020** 9000 000A** 0000","00 1E 6F 1C 84 07 A0 00 00 00 04 10 10 A5 11 50 0F 50 50 43 20 4D 43 44 20 30 31 20 20 76 32 30"])
C2C03.append(["GPO ","2C 03 80 A8 00 00 02 83 00 00 ","56 69 56 4F 74 65 63 68 32 00 2C00 0032 0020** 9000 000A** 0000","00 18 77 16 82 02 59 80 94 10 08 01 01 00 10 01 01 01 18 01 02 00 20 01 02 00"])
C2C03.append(["Read Record ","2C 03 00 B2 01 0C 00 ","56 69 56 4F 74 65 63 68 32 00 2C00 009A 0088** 9000 000A** 0000","00 7F 70 7D 9F 6C 02 00 01 9F 62 06 00 00 00 38 00 00 9F 63 06 00 00 00 00 E0 E0 56 3C 42 35 34 31 33 33 33 30 30 38 39 36 30 30 30 31 30 5E 45 54 45 43 2F 50 41 59 50 41 53 53 5E 31 34 31 32 32 30 31 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 30 9F 64 01 03 9F 65 02 00 0E 9F 66 02 0E 70 9F 6B 13 54 13 33 00 89 60 00 10 D1 41 22 01 90 00 99 00 00 00 0F 9F 67 01 03"])
C2C03.append(["CCC","2C 03 80 2A 8E 80 04 00 00 00 00 00","56 69 56 4F 74 65 63 68 32 00 2C00 002A 0018** 9000 000A** 0000","00 11 77 0F 9F 60 02 A0 A9 9F 61 02 D9 B1 9F 36 02 00 02"])

CC733=[]
CC733.append(["Get Dukpt Encryption","C7 33","C7 00 00 01 00"])
CC733.append(["Get Dukpt Encryption-TransarmorTDES","C7 33","C7 00 00 01 02"])
CC7A3 = []
CC7A3.append(["Get DataKey Encrp Type","C7 a3 01 00"," C7 00 00 06 00 00"])
CC7A3.append(["Get Dukpt Encryption-TransarmorTDES","C7 a3 01 00","C7 00 00 06 00 02"])
CC732=[]
CC732.append(["Set to TransArmor TDES","C7 32 02","C7 00"])
CC732.append(["Set to TDES","C7 32 00","C7 00"])
CC7A2=[]
CC7A2.append(["Set to TransArmor TDES","C7 A2 01 00 00 02 00 00 00 00","C7 00"])
CC7A2.append(["Set to TransArmor TDES","C7 A2 01 00 00 00 00 00 00 00","C7 00"])
...
#Tra-TDES + MAC off + CustomerID2
if(False ==  Pro_SendCmd().RunCK(Pro_SendCmd().Run(CC7A3[1][0],CC7A3[1][1],30000,False,DL),CC7A3[1][2],DL) and False ==  Pro_SendCmd().RunCK(Pro_SendCmd().Run(CC733[1][0],CC733[1][1],30000,False,DL),CC733[1][2],DL)): 
	...
	if(False ==  Pro_SendCmd().RunCK(Pro_SendCmd().Run(CC7A2[0][0],CC7A2[0][1],30000,False,DL),CC7A2[0][2],DL) and False ==  Pro_SendCmd().RunCK(Pro_SendCmd().Run(CC732[0][0],CC732[0][1],30000,False,DL),CC732[0][2],DL)): DL.warnings=DL.warnings+1
else:
...
for case_2C03 in C2C03:
    strRes = Pro_SendCmd().Run(case_2C03[0],case_2C03[1],5000,False,DL)
    if(False ==  Pro_SendCmd().RunCK_C(strRes,case_2C03[2],DL)):DL.fails = DL.fails + 1
    ListAPDU = Pro_Encryption().DecRAPDU(strRes,strKey,0,1,True,False,case_2C03[3],DL)
...
#Set back to TDES
Pro_SendCmd().Run(CC732[1][0],CC732[1][1],3000,False,DL)
Pro_SendCmd().Run(CC7A2[1][0],CC7A2[1][1],3000,False,DL)
##################################################
