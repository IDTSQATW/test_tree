...
for Case in Site:
	if (False == Pro_SendCmd().RunCK(Pro_SendCmd().Run("Read card profile"+Case[0]+" before delete", "CE 03"+Case[0], 3000, False, DL),Case[3], DL)): DL.warnings = DL.warnings+ 1
	if (False == Pro_SendCmd().RunCK(Pro_SendCmd().Run("Delete card profile"+Case[0], "CE 04"+Case[0], 3000, False, DL),"CE 00 00 00", DL)): DL.fails = DL.fails + 1
	if (False == Pro_SendCmd().RunCK(Pro_SendCmd().Run("Delete card profile AGAIN", "CE 04"+Case[0], 3000, False, DL),"CE 0A 00 00", DL)): DL.fails = DL.fails + 1
	if (False == Pro_SendCmd().RunCK(Pro_SendCmd().Run("Read card profile"+Case[0]+" after delete", "CE 04"+Case[0], 3000, False, DL),"CE 0A 0000", DL)): DL.fails = DL.fails + 1
	if (False == Pro_SendCmd().RunCK(Pro_SendCmd().Run("Select card profile"+Case[0], "CE 05"+Case[0], 3000, False, DL),"CE 0A 00 00", DL)): DL.fails = DL.fails + 1
if (DL.warnings > 0): DL.setText("red","Please ensure FW.K.10.01-02_CE-XXCardEmulation_CreateCard.py and FW.K.10.01-03_CE-XXCardEmulation_UpdateCard.py ran and succsss before this case")
