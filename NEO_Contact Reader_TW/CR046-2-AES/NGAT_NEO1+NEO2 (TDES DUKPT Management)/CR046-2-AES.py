		...
		if (Result):
			if readertype == 0:
				...
				if (Result):
					...
					if (Result):
						Result = DL.Check_StringAB(DL.Get_RXResponse(5), '4A DF EE 25')
				sResult=DL.Get_RXResponse(5)
			if readertype == 1:		
				Result = DL.Check_StringAB(DL.Get_RXResponse(3), '56 69 56 4F 74 65 63 68 32 00 60 00')
				if (Result):
					Result = DL.Check_StringAB(DL.Get_RXResponse(3), '6A DF EE 25')
...
if readertype == 1:
	...
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("56 69 56 4F 74 65 63 68 32 00 60 00 ** 6A 57 00 5A 00 5F 34 00 5F 20 00 5F 24 00 9F 20 00 5F 25 00 5F 2D 00 50 00 4F 00 84 00 DF EE 23 00 9F 39 00")
if readertype == 0:
	RetOfStep = DL.SendCommand('60-13 Contact Retrieve Transaction Result')	
	if (RetOfStep):
