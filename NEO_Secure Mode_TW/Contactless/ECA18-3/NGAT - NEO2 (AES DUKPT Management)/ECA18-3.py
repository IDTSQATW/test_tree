...
if (Result):
	...
	if (RetOfStep):
        ...
        if Result == False: 
            DL.fails=DL.fails+1
            DL.SetWindowText("Red", "Expected result: C7 00 00 06 01 02 00 00 00 00")
        ...
        if (Result):
            ...
            if (Result):
                for i in range(1, pollmode):
                    ...
                    if i == 2: RetOfStep = DL.SendCommand('Auto Poll')
                    if (RetOfStep):
                        Result = DL.Check_RXResponse("01 00 00 00")
                        if Result == False: 
                            DL.fails=DL.fails+1
                            DL.SetWindowText("Red", "Expected result: 01 00 00 00")
                    ...
                    if (Result):
                        ...
                        if (RetOfStep):
                            ...
                            if i == 2:
                                rx = 1
                                alldata = DL.Get_RXResponse(rx)	
                                Result = DL.Check_RXResponse(rx, '56 69 56 4F 74 65 63 68 32 00 03 23 ** E5 ** DF EE 12')
                            if (Result):
