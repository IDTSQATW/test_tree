...
if (Result):
	RetOfStep = DL.SendCommand('60-10 Contact Start Transaction (Not support FallBack)_1')
	if (RetOfStep):
...
if (Result):
	RetOfStep = DL.SendCommand('60-10 Contact Start Transaction (Not support FallBack)_2')
	if (RetOfStep):
		...
		if (Result):
			...
			if (Result):
				Result = DL.Check_StringAB(DL.Get_RXResponse(1), 'E0 DF EE 12')
			if (Result):
...
if  CTresultcode == "0004":
	...
	if (RetOfStep):
		...
		if (Result):
			Result = DL.Check_StringAB(DL.Get_RXResponse(1), '56 69 56 4F 74 65 63 68 32 00 60 00')
			if (Result):
				Result = DL.Check_StringAB(DL.Get_RXResponse(1), 'E0 DF EE 25')
			if (Result):
				ksn = DL.GetTLV(alldata,"DFEE12")
