...
if(Robot == False):	DL.ShowMessageBox("card","Please Tap S50/S70 card after clicking, not remove until the end",0)
...
if(False ==  Pro_SendCmd().RunCK(Pro_SendCmd().Run(C2C04[1][0],C2C04[1][1],5000,False,DL),C2C04[1][2],DL)): DL.fails = DL.fails +1
if(False ==  Pro_SendCmd().RunCK(Pro_SendCmd().Run(C2C04[2][0],C2C04[2][1],5000,False,DL),C2C04[2][2],DL)): DL.fails = DL.fails +1
