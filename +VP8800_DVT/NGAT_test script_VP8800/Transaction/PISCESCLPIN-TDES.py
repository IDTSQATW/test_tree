...
Result = DL.SendCommand("Activate Transaction(CL)")
sResult = DL.Get_RXResponse(0)

Result = DL.SendCommand("Display Online Authorization Result(OK)")
sResult = DL.Get_RXResponse(0)

Result = DL.SendCommand("Get Online Enciphered PIN(Format0 TDES)")
sResult = DL.Get_RXResponse(0)
...
if sResult!=None and sResult!="":
    ...
    EncOlPIN_Hex=sResultData[0:32]
    PINKSN_Hex=sResultData[32:72]
