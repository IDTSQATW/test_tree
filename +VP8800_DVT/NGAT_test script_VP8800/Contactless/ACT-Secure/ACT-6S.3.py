...
if (Result):
	...
	if (RetOfStep):
		DL.Check_RXResponse("56 69 56 4F 70 61 79 56 33 00 02 05 23 ?? ?? D1 DF EF 18 A1 25 35 34 31 33 31 32 2A 2A 2A 2A 2A 2A 34 38 30 38 3D 2A 2A 2A 2A 31 30 31 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A DF EF 18 C1 30 ** 9F 6B A1 13 54 13 12 CC CC CC CC CC CC CC CC CC CC CC CC CC C3 21 0F 9F 6B C1 18 ")
...
if (Result):
	...
	if (RetOfStep):
		DL.Check_RXResponse("56 69 56 4F 70 61 79 56 33 00 02 05 23 ?? ?? C1 50 0C 56 49 53 41 20 54 45 53 54 20 30 31 57 A1 13 47 61 73 CC CC CC 00 10 DC CC C1 20 CC CC CC CC CC CC CC 57 C1 18 ** 5A A1 08 47 61 73 CC CC CC 00 10 5A C1 10")
		str1 = DL.Get_RXResponse(1)
...
if (Result):
	...
	if (RetOfStep):
		DL.Check_RXResponse("56 69 56 4F 70 61 79 56 33 00 02 05 0A ?? ?? 54 90 00 05 FF 81 05 82 01 6E 50 10 41 6D 65 72 69 63 61 6E 20 45 78 70 72 65 73 73 57 A1 13 37 42 45 CC CC C0 00 1D CC CC 70 2C CC CC CC CC CC CC CC 57 C1 18 ** 5A A1 08 37 42 45 CC CC C0 00 1F 5A C1 10")
...
if (Result):
	...
	if (RetOfStep):
		DL.Check_RXResponse("56 69 56 4F 70 61 79 56 33 00 02 05 23 ?? ?? D1 DF EF 18 A1 24 35 34 31 33 33 33 2A 2A 2A 2A 2A 2A 31 35 31 33 3D 2A 2A 2A 2A 32 30 31 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A DF EF 18 C1 28 ** 5A A1 08 54 13 33 CC CC CC 15 13 5A C1 10 ** 57 A1 12 54 13 33 CC CC CC 15 13 DC CC C2 01 CC CC CC CC CC CC 57 C1 18")
		str1 = DL.Get_RXResponse(1)
		str2 = DL.GetTLV(str1,"DFEF18", 1)
		str3 = DL.GetTLV(str1,"DFEE12")
		str4 = DL.DecryptDLL(0,1, strKey, str3, str2)
		...
		str7 = DL.DecryptDLL(0,1, strKey, str3, str6)
...
if (Result):
	...
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("56 69 56 4F 70 61 79 56 33 00 02 05 0A 01 93 42 90 00 0A ** 57 A1 11 54 13 33 CC CC CC 15 13 DC CC C2 20 CC CC CC CC CC 57 C1 18 ** 5A A1 08 54 13 33 CC CC CC 15 13 5A C1 10")
		str = DL.Get_RXResponse(1)
		str=str.replace(" ","")
		str1 = str[38:len(str) - 4]
		str2 = DL.GetTLV(str1,"57", 1)
...
if (Result):
	...
	if (RetOfStep):
		DL.Check_RXResponse("56 69 56 4F 70 61 79 56 33 00 02 05 23 ?? ?? D1 DF EF 17 A1 41 2A 36 30 31 31 30 30 2A 2A 2A 2A 2A 2A 34 32 37 31 5E 44 49 53 43 4F 56 45 52 2F 4E 45 54 57 4F 52 4B 20 20 20 20 20 20 20 20 20 20 5E 2A 2A 2A 2A 31 30 31 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A DF EF 17 C1 48 ** DF EF 18 A1 25 36 30 31 31 30 30 2A 2A 2A 2A 2A 2A 34 32 37 31 3D 2A 2A 2A 2A 31 30 31 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A DF EF 18 C1 30 ** 56 A1 41 2A 36 30 31 31 30 30 2A 2A 2A 2A 2A 2A 34 32 37 31 5E 44 49 53 43 4F 56 45 52 2F 4E 45 54 57 4F 52 4B 20 20 20 20 20 20 20 20 20 20 5E 2A 2A 2A 2A 31 30 31 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 56 C1 48 ** 57 A1 13 60 11 00 CC CC CC 42 71 DC CC C1 01 CC CC CC CC CC CC CC 57 C1 18 ")
		str1 = DL.Get_RXResponse(1)
		...
		str11 = DL.DecryptDLL(0,1, strKey, str3, str10)
		...
		str13 = DL.DecryptDLL(0,1, strKey, str3, str12)
