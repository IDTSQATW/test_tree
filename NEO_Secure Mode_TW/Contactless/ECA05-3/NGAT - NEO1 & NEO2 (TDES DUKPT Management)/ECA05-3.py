...
if (Result):
	...
	if (RetOfStep):
		...
		if platform == 1: #NEOII and upward
			Result = DL.Check_RXResponse(rx, "56 69 56 4F 74 65 63 68 32 00 02 23 ** F3 ** DF EE 12")
			ksn = DL.GetTLV(alldata,"DFEE12")	
		if platform == 0: #NEOI
			Result = DL.Check_RXResponse(rx, "56 69 56 4F 74 65 63 68 32 00 02 23 ** D3 ** FFEE12")
		...
		if (Result):
            ...
            # Tag 56
			if platform == 0: #NEOI
				Result = DL.Check_RXResponse(rx, '56 A1 29 2A 35 31 32 38 2A 2A 2A 2A 2A 2A 2A 2A ** 5E 20 2F 5E 31 38 30 33 363232 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A 2A')
            ...
            # Tag 9F6B
			if platform == 0: #NEOI
				Result = DL.Check_RXResponse(rx, '9F 6B A1 13 51 28 CC CC CC CC ** D1 80 3 622 CC CC CC CC CC CC CC')
			else: #NEOII and upward
			...
			if platform == 1: #NEOII and upward
				Result = DL.Check_RXResponse(rx, "DFEE26 02 F300")
			if platform == 0: #NEOI
				Result = DL.Check_RXResponse(rx, "DFEE26 01 D3")
			if Result == False:
				DL.fails=DL.fails+1
