...
if (Result):
	RetOfStep = DL.SendCommand('Get DUKPT KSN (81-0B)_0205')
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("62 99 49 01 50 00 00 00")
...
# Set MasterCard CT config
if (Result):
	RetOfStep = DL.SendCommand('Set MasterCard CT config (60-03)')
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("00")
...
if (Result):
	...
	# Tag 5A
		Result = DL.Check_StringAB(mask5A, '5A A1 0A 67 99 CC CC CC CC CC C0 01 0F')
		if Result == True and DL.Check_RXResponse("5A A1 0A"):
			DL.SetWindowText("blue", "Tag 5A_Mask: PASS")
		else:
			DL.SetWindowText("red", "Tag 5A_Mask: FAIL")
			
		Result = DL.Check_StringAB(dec5A, '5A 0A 67 99 99 89 00 00 02 00 01 0F')
	...
	# Tag 57
		...
		Result = DL.Check_StringAB(dec57, '57 12 67 99 99 89 00 00 02 00 01 0D 25 12 62 00 62 93 04 23')
