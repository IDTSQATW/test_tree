...
if (Result):
	RetOfStep = DL.SendCommand('(C7-6D) Load Campus Card Key Block / Retrieve Key Block Info')
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("C7 00 00 2D")
...
if (Result):
	for j in range (1, 7):
		...
		if (RetOfStep):
			Result = Result and DL.Check_RXResponse("02 00 ** 21")
			if (Result):
				...
				Result = DL.Check_StringAB(decDFEF4D, '39 39 39 38 38 38 30 32 32 36 32 30 30 35 39 39')
				if Result == True:
