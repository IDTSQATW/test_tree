...
PINKey1 = '11111111111111111111111111111111'

# Check PIN key slot 01
if (Result):
	RetOfStep = DL.SendCommand('Get DUKPT KSN (81-0B)_slot 0101 (PIN key)')
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("62 99 49 02 10 00 00 00")

# Set IIN
if (Result):
	RetOfStep = DL.SendCommand('Set IIN 651000 PIN slot 01')
	if (RetOfStep):
...
if (Result):
	...
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("01 03 65 10 00 01 09")

# cmd 02-05, Swipe card
if (Result):
...
# Get online-enciphered PIN
if (Result):
	RetOfStep = DL.SendCommand('Get online-enciphered PIN')
	if (RetOfStep):
		DL.Check_RXResponse("56 69 56 4F 74 65 63 68 32 00 83 00 00 24")
		if(Result):
                    strResponse = DL.Get_RXResponse(0)
                    strResponse = strResponse.replace(" ","")
                    strGetPINEncdata = DL.getASCIIArray(strResponse[28:len(strResponse) - 4])
                    strPINEncdata = strGetPINEncdata[0:16]
                    strPINKSN = strGetPINEncdata[16:36]
                    strPINDecdata = DL.DecryptDLL(1, 1, PINKey1, strPINKSN, strPINEncdata)

                    if(strGetPINEncdata != ''):
                       Result = True
                    DL.setText("blue", 'Transit to ASCII:[' + strGetPINEncdata + ']')
                    DL.setText("blue", 'Encrypted PIN data:[' + strPINEncdata + ']')
                    DL.setText("blue", 'KSN for PIN key:[' + strPINKSN + ']')
                    DL.setText("blue", 'Decrypted PIN key:[' + strPINDecdata + ']')


                    Result = DL.Check_StringAB(strPINDecdata, "1234")
                    if Result != True:
                        DL.SetWindowText("red", "PIN Key encryption Fail")
