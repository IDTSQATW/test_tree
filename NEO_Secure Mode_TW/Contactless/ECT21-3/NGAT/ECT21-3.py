...
# Objective: to verify TransArmor AES
...
# Encryption Type -- TransArmor AES
if (Result):
	RetOfStep = DL.SendCommand('C7-A2 AES DUKPT manage_AES-128 with TransArmor AES, Data key both ways')
	time.sleep(1)
...
if (Result):
	...
	if (RetOfStep):
		Result = Result and DL.Check_RXResponse("C7 00 00 06 01 05 00 00 00 00")
...
if (Result):
    ...
    if (RetOfStep):
        ...
        if (Result):
            ...
            Result = DL.Check_StringAB(dec57, '3B343736313733393030313031303031303D33303132313230303031323333393930303033313F000000000000000000')
            if Result == False:
                Result = DL.Check_StringAB(dec57, '3B343736313733393030313031303031303D32303132313230303031323333393930303033313F000000000000000000')
            if Result == True and DL.Check_RXResponse(rx, "57 C1 30"):
            ...
            if DL.Check_RXResponse(rx, "DFEE26 02 E507") == False: 
                DL.fails=DL.fails+1
